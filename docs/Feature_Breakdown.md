...
**Workflow:**
1. **Pass 1 (Timestamp Gap):** Images sorted by filename timestamp. Groups formed when gap > 60 seconds.
2. **Pass 1.5 (Split Overloaded):** Groups with multiple FRONTs or BACKs are split into separate groups.
3. **Pass 2 (Heuristic Fuzzy):** Ungrouped images matched via fuzzy scoring (color similarity, garment type matching, pattern similarity, filename proximity). Minimum threshold: 1.0 for new groups, 0.4 for low-confidence.
4. **Pass 3 (Vision Confirm Solos):** Solo (single-image) groups sent to AI with neighbor grid to confirm assignment.
5. **Pass 4 (Vision Confirm Suspicious):** Groups with suspicious characteristics (multiple garment types, duplicate FRONTs) sent to AI for confirmation/splitting.
